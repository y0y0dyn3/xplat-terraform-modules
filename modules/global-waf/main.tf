...
resource "aws_waf_ipset" "iplist_throttle" {
    name  = "${var.stage}_${var.api_name}_iplist_throttle"
...
resource "aws_waf_ipset" "iplist_blacklist" {
    name  = "${var.stage}_${var.api_name}_iplist_blacklist"
...
resource "aws_waf_xss_match_set" "xss_match_conditions" {
    name  = "${var.stage}_${var.api_name}_xss_match_conditions"
...
resource "aws_waf_sql_injection_match_set" "sql_injection_match_set" {
  name = "${var.stage}_${var.api_name}_sql_injection_match_set"
...
resource "aws_waf_byte_match_set" "byte_set_traversal" {
  name  = "${var.stage}_${var.api_name}__byte_match_set"
...
resource "aws_waf_byte_match_set" "byte_set_webroot_requests" {
    name  = "${var.stage}_${var.api_name}__byte_match_webroot_requests"
...
resource "aws_waf_rule" "ip_blacklist" {
    ...
    name        = "${var.stage}_${var.api_name}_ip_blacklist"
    ...
    predicate {
        type    = "IPMatch"
        data_id = "${aws_waf_ipset.iplist_blacklist.id}"
        negated = false
    }
...
resource "aws_waf_rate_based_rule" "rate_ip_throttle" {
    ...
    name        = "${var.stage}_${var.api_name}_ip_throttle"
    # metric_name is alpha numeric only.
    ...
    predicate {
        type    = "IPMatch"
        data_id = "${aws_waf_ipset.iplist_throttle.id}"
        negated = false
    }

    depends_on = ["aws_waf_ipset.iplist_throttle"]
...
resource "aws_waf_rule" "xss_match_rule" {
    name        = "${var.stage}_${var.api_name}_xss_match_rule"
    # metric_name is alpha numeric only.
    metric_name = "${replace(var.stage, "/[^a-zA-Z0-9_]/", "")}${replace(var.region, "/[^a-zA-Z0-9_]/", "")}${replace(var.api_name, "/[^a-zA-Z0-9_]/", "")}xssmatchrule"

    predicate {
        type    = "XssMatch"
        data_id = "${aws_waf_xss_match_set.xss_match_conditions.id}"
        negated = false
    }

    depends_on = ["aws_waf_xss_match_set.xss_match_conditions"]
...
resource "aws_waf_rule" "sql_injection_rule" {
    name        = "${var.stage}_${var.api_name}_sql_injection_rule"
    # metric_name is alpha numeric only.
...
resource "aws_waf_rule" "byte_match_traversal" {
  ...
  name        = "${var.stage}_${var.api_name}_byte_match_traversal"
  # metric_name is alpha numeric only.
  ...
  predicate {
      type    = "ByteMatch"
      data_id = "${aws_waf_byte_match_set.byte_set_traversal.id}"
      negated = false
  }

    depends_on = ["aws_waf_byte_match_set.byte_set_traversal"]
...
resource "aws_waf_rule" "byte_match_webroot" {
  ...
  name        = "${var.stage}_${var.api_name}_byte_match_webroot"
  # metric_name is alpha numeric only.
  ...
  predicate {
      type    = "ByteMatch"
      data_id = "${aws_waf_byte_match_set.byte_set_webroot_requests.id}"
      negated = false
  }

    depends_on = ["aws_waf_byte_match_set.byte_set_webroot_requests"]
...
resource "aws_waf_web_acl" "rms_web_acl" {
    name        = "${var.stage}_${var.api_name}_rms_web_acl"
    # metric_name is alpha numeric only.
    ...
    depends_on  = [
        ...
        "aws_waf_rule.byte_match_traversal",
        "aws_waf_rule.xss_match_rule",
        "aws_waf_rule.byte_match_webroot",
        "aws_waf_rule.sql_injection_rule"
    ]
    ...
    rule {
        ...
        priority  = 40
        rule_id   = "${aws_waf_rule.byte_match_traversal.id}"
        type      = "REGULAR"
    }
        ...
        priority  = 50
        rule_id   = "${aws_waf_rule.byte_match_webroot.id}"
        type      = "REGULAR"
    }
...
resource "aws_waf_web_acl_association" "web_acl_association" {
    resource_arn  = "${var.acl_association_resource_arn}"
    web_acl_id    = "${aws_waf_web_acl.rms_web_acl.id}"
    count         = "${var.enabled}"
}
